...
export function FlowEditor() {
  ...
  return <div className={`phase2FlowEditor view-${view}`}>
    ...
    {selectedStep && <StepConfigurationPanel step={selectedStep} onClose={() => setSelectedStepId(null)} onSummaryChange={updateSummary} />}
...
function Phase2TokenPicker({ onClose }: { onClose: () => void }) {
  const [tab, setTab] = useState('program')
  const tokens: Record<string, string[]> = { program: ['{{my.EventTitle}}', '{{my.EventDate}}', '{{my.OfferName}}'], person: ['{{Person.FirstName}}', '{{Person.Company}}', '{{Person.Email}}', '{{Person.Score}}'], system: ['{{System.Date}}', '{{System.Year}}', '{{System.SenderName}}'], trigger: ['{{Trigger.FormName}}', '{{Trigger.ClickLink}}'] }
